...
| | 旧版（1.py） | 新版（NX-Jikkyo 模型） |
|---|---|---|
| 滚动速度 | 固定 320px/s（长弹幕停留 12s+） | **固定 5s 跨屏**（长弹幕更快） |
| 行数 | 固定 24 行 | **播放器高 ÷ 40，随窗口变化** |
| 字号 | 60px | **34px**（宽≥819px 不缩放） |
| big/small | 1.5× / 0.5× | **1.25× / 0.8×**（不影响行高） |
| 颜色 | 随机/近似色 | **NX-Jikkyo 官方色表**（默认粉白 #FFEAEA） |
| 时间轴 | vpos 或平移到 0 | **chat.date − 视频开始时间**（源码同款公式） |
